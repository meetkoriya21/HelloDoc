...
const TopDoctors = () => {
  ...
  return (
    <div className='flex flex-col items-center gap-4 my-16 text-grey-900 md:mx-10'>
      ...
      <div className='w-full grid grid-cols-auto   gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
        {doctors.slice(0,5).map((item,index)=>(
            <div onClick={()=>{navigate(`/appointment/${item._id}`); scrollTo(0,0)}} className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500' key={index} >
                <img className='bg-blue-50 h-52 w-64' src={item.profilePicture} alt="" />
                <div className='p-4'>
      ...
      <button onClick={()=>{navigate('/alldoctors'); scrollTo(0,0)}} className='bg-blue-500 text-white px-12 py-3 rounded-full mt-10 hover:scale-105 transition-all'>More</button>
    </div>
  )
}
